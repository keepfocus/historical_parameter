...
class HistoricalEditFormTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    stub(@template).polymorphic_path { "" }
    stub(@template).protect_against_forgery? { false }
  end  

  def document_root_element
    Nokogiri::HTML(@output).root
  end
  ...
  test "after_historical_form content comes after form" do
    ...
    assert_select "form ~ span#before"
    assert_select "form ~ span#inside"
    assert_select "form span#before", false
    assert_select "form span#inside", false
  end
  ...
  test "historical_value_fields inserts fields required for a value" do
    ...
    assert_select "input[name=?][type=text]", "area_history[value]"
    assert_select "select[name=?]", "area_history[valid_from(1i)]"
    assert_select "select[name=?]", "area_history[valid_from(2i)]"
    assert_select "select[name=?]", "area_history[valid_from(3i)]"
    assert_select "input[name=?][type=checkbox]", "area_history[_destroy]"
  end
  ...
  test "new_history_value_button is inserted into form and template outside" do
    ...
    assert_select "form.new_installation" do
    ...
    assert_select "table#area_history_fields_template" do
  ...
  test "history_edit_table_for should create table for editing values over time" do
    ...
    assert_select "table#area_history_table > tbody" do
      assert_select "tr", 2
